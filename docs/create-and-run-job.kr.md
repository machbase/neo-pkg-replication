...
저장 전에 시스템이 dry-run 검증을 수행할 수 있습니다.  
문제가 치명적이지 않지만 확인이 필요한 경우에는 **Validation Warnings** 창이 뜨고, 사용자는 다음 중 하나를 선택합니다.
...
일반적으로는 Job을 생성하면 바로 Replication이 시작되므로, 먼저 대시보드와 사이드바에서 상태를 확인하면 됩니다.

확인할 항목:

1. 사이드바에서 해당 Job이 목록에 나타나는지 확인합니다.
2. 상태가 `running` 또는 대시보드의 `REPLICATING` / `IDLE STATE`로 보이는지 확인합니다.
