...
│   ├── job.js                    # JobScheduler, Job, AbortController (직접 구현)
│   ├── api/
│   │   └── http_server.js        # HttpServer -- REST API (jsh 내장 http 모듈, JobScheduler에만 의존)
│   ├── config/
...
- jsh 내장 `http` 모듈 사용 (`new http.Server({network: 'tcp', address: 'host:port'})`)
