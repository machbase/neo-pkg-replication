...
/** @type {{ host: string, port: number, database: string, user: string, password: string }} 소스 DB 접속 정보 */
const SRC = {
  host: readEnv('RPL_TEST_SRC_HOST', '127.0.0.1'),
  port: readEnvInt('RPL_TEST_SRC_PORT', 5656),
  database: readEnv('RPL_TEST_SRC_DATABASE', 'MACHBASEDB'),
  user: readEnv('RPL_TEST_SRC_USER', 'SYS'),
  password: readEnv('RPL_TEST_SRC_PASSWORD', 'MANAGER'),
};

/** @type {{ host: string, port: number, database: string, user: string, password: string }} 대상 DB 접속 정보 */
const DST = {
  host: readEnv('RPL_TEST_DST_HOST', SRC.host),
  port: readEnvInt('RPL_TEST_DST_PORT', SRC.port),
  database: readEnv('RPL_TEST_DST_DATABASE', SRC.database),
