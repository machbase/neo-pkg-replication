...
- `createTagTable(tableName, schema)`: `TableSchema` 기반으로 TAG 테이블 DDL 생성 후 실행
...
- `createLogTable(tableName, schema)`: `TableSchema` 기반으로 LOG 테이블 DDL 생성 후 실행
...
```js
CheckpointStore.load(dataTable) → { cp, exists, err }
CheckpointStore.save(dataTable, cp, stats) → err
```

**구현 항목**
- atomic write 내장 (tmp 파일 → rename). fs 동기 API 사용. 헬퍼 함수(`_stringify`, `_writeFile`)로 내장.
