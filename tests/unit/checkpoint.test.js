...
test('save + load 라운드트립: BigInt rid 보존', async () => {
  ...
  const err = await store.save('job-1', '_TAG_DATA_0', { lastSuccessRid: rid }, {});
  assert.equal(err, null);

  const result = await store.load('job-1', '_TAG_DATA_0');
  assert.equal(result.exists, true);
  assert.equal(result.err, null);
  assert.equal(result.cp.lastSuccessRid, rid);
});

test('save + load: sourceServer, sourceTable 보존', async () => {
  const dir = tmpDir();
  await fs.mkdir(dir, { recursive: true });
  const store = new CheckpointStore(dir);
  ...
  await store.save('job-1', '_TAG_DATA_1', {
    ...
    sourceServer: 'src',
    sourceTable: 'TAG',
  }, { rowsRead: 10, rowsWritten: 9, droppedNoMeta: 1, skippedExists: 0 });

  const result = await store.load('job-1', '_TAG_DATA_1');
  assert.equal(result.exists, true);
  assert.equal(result.cp.lastSuccessRid, 42n);
});

test('load: source.dataTable 불일치 → { exists: false, err }', async () => {
  const dir = tmpDir();
  await fs.mkdir(dir, { recursive: true });
  const store = new CheckpointStore(dir);

  // 정상 저장 후 파일 내부 dataTable 값을 오염
  await store.save('job-1', '_TAG_DATA_0', { lastSuccessRid: 0n }, {});
  const filePath = path.join(dir, 'job-1_TAG_DATA_0.json');
  const raw = JSON.parse(await fs.readFile(filePath, 'utf-8'));
  raw.source.dataTable = '_TAG_DATA_CORRUPTED';
...
test('rid = 0n 저장 및 로드', async () => {
  ...
  await store.save('job-2', '_TAG_DATA_0', { lastSuccessRid: 0n }, {});
  const result = await store.load('job-2', '_TAG_DATA_0');
  assert.equal(result.cp.lastSuccessRid, 0n);
});
